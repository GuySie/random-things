...
bool HOT EPaperT133A01::transfer_data() {
  ...
  if (half < total_rows) {
    ...
    while (half < total_rows) {
      ...
      // Yield only between rows, never after the final row: returning here with
      // the phase complete would skip the disable()/CS-release epilogue below on
      // the next call, leaving the SPI transaction open and deadlocking the next
      // enable() (task watchdog reboot).
  ...
  if (half >= total_rows && half < total_rows * 2) {
    ...
    while (half < total_rows * 2) {
      ...
      // Same as the CS phase: never yield after the final row, or the epilogue
      // below is skipped and the transfer "completes" with CS1 still asserted.
