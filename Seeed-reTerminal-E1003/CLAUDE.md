...
- `on_shutdown` has two handlers: priority 200 disables the battery voltage divider output (`bsp_battery_enable`) before any shutdown path; priority 100 computes the clock-aligned sleep duration from the PCF8563 RTC — it finds the next boundary at `rtc_interval_hours`-hour intervals, advances past any boundaries outside `rtc_hour_start`–`rtc_hour_end`, and falls back to one full interval if the RTC is not set. Computing at shutdown (rather than boot) accounts for actual time spent awake and gives more accurate wake-up alignment.
