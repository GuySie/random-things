...
- `wifi.fast_connect: true` is set so the device skips active scanning and connects faster on known networks — important for minimising active wake time.
- `wifi.on_connect` triggers `component.update: sht40_sensor` (reads temperature for IT8951 waveform selection; completes in ~10 ms, well before the download finishes), `component.update: battery_voltage` (so battery level is read once per wake cycle while wifi is active), and then `random_image` button, which picks an image (mode controlled by the `use_image_history` switch — see below), sets `image_counter`, which triggers `online_image.set_url`, which triggers a download. On download completion, `component.update: epaper_display` is triggered, the image number is recorded to the history buffer, and then `deep_sleep.enter` is called directly. On download error, `deep_sleep.enter` is also called directly. This means the device enters deep sleep from the `online_image` callbacks rather than waiting for any external HA trigger.
- `on_shutdown` has three handlers: priority 200 disables the battery voltage divider output (`bsp_battery_enable`) before any shutdown path; priority 100 computes the clock-aligned sleep duration from the PCF8563 RTC — it finds the next boundary at `rtc_interval_hours`-hour intervals, advances past any boundaries outside `rtc_hour_start`–`rtc_hour_end`, and falls back to one full interval if the RTC is not set. Computing at shutdown (rather than boot) accounts for actual time spent awake and gives more accurate wake-up alignment. Priority 50 runs the GPIO hold sequence (see power optimisation note below).
- Deep sleep is gated by a Home Assistant `input_boolean` synced via the `allow_deepsleep` homeassistant switch. The refresh schedule (`Refresh Interval`, `Active Hours Start`, `Active Hours End`) is configurable from HA without reflashing via persistent `number` entities (defaults: 1 h interval, 07:00–23:00 window). The green button (GPIO3) wakes the device via `esp32_ext1_wakeup` (ANY_LOW) — ext1 is required because `gpio_deep_sleep_hold_en()` breaks ext0 wakeup on the S3.
- The image subfolder is set via a persistent `text` entity (`album_folder_text`, "Album Folder") editable from HA at runtime. The initial value is `""` (images served from the www root). No trailing slash is needed — the URL lambda appends one automatically when the folder is non-empty.
...
- `on_boot` has two handlers: priority 900 calls `gpio_hold_dis` on all held pins (GPIO11, 21, 16, 38, 39, 45, 48) before the IT8951 driver's HARDWARE-priority (800) `setup()` runs its `power_cycle_()` routine — that routine drives GPIO11 and GPIO21 LOW then HIGH on every boot, which requires the holds to be released first. Priority 600 enables the battery voltage divider and triggers `component.update: last_boot_sensor`. The `last_boot_sensor` is a `text_sensor` (platform: template, `device_class: timestamp`, `update_interval: never`) that reads `id(rtc_time).now()` and formats it as a UTC ISO 8601 string (`%Y-%m-%dT%H:%M:%S+00:00`). Because the PCF8563 is coin-cell-backed and set to UTC, the boot timestamp is available immediately without waiting for HA time sync. Returns `{}` (unavailable) if the RTC VL flag is set (first boot before any time sync).

**Deep sleep power optimisation** — without GPIO holds, measured sleep current is ~5 mA (draining ~3%/day). The root causes are GPIO39 (SD_EN, U13 TPS22916 load switch) and GPIO48 (TOUCH_RES, GT911 touch controller reset, active-low) floating HIGH during sleep because bare `digitalWrite()` calls are silently discarded at sleep entry on the ESP32-S3. The fix: `on_shutdown` priority 50 calls `gpio_hold_en()` on GPIO11, 21, 38, 39, 45, 48 (GPIO16/USER_LED excluded — latching it LOW drives the LED on via Q4 transistor, causing stuck-on LED during sleep), then `gpio_deep_sleep_hold_en()` to arm the hold domain for non-RTC pins (>GPIO21). `<driver/rtc_io.h>` (available at `~/.esphome/platformio/packages/framework-arduinoespressif32/tools/sdk/esp32s3/include/driver/include/driver/rtc_io.h`) is included for `rtc_gpio_pullup_en`/`rtc_gpio_pulldown_dis` on the wakeup pin (GPIO3). With holds applied, sleep current drops to sub-milliamp (below BM235 meter resolution). Verified: the `[power]` BEFORE/AFTER log lines at shutdown show IO39 and IO48 transitioning from 1 to 0. Awake current is ~185 mA; a full wake cycle (connect, download, render, sleep) takes ~90 s, so awake energy is a small fraction of the daily budget.

Fixes confirmed to have **no effect** on sleep current (do not re-investigate these): bare `digitalWrite()` LOW on GPIO21 or GPIO11 before sleep (silently discarded by ESP32-S3 at sleep entry); switching to ext1 wakeup alone without holds; holding GPIO21/GPIO11 only (IT8951 display rail is negligible at idle — zero change from 4.95 mA baseline in testing); GPIO46 (Header_EN) is already LOW by default.

The irreducible hardware quiescent floor is ~90 µA: SY6974B charger ~75 µA + TPS631000DRLR buck-boost ~15 µA + PCF8563 RTC ~250 nA. These are always-on and not firmware-addressable. Potential further optimisation: ESP-IDF sdkconfig flags `CONFIG_ESP_SLEEP_PSRAM_LEAKAGE_WORKAROUND` and `CONFIG_ESP_SLEEP_FLASH_LEAKAGE_WORKAROUND` (not currently applied).
...
- **0x51** — PCF8563 RTC
- **0x5D** — GT911 touch controller (TOUCH_RES=GPIO48 active-low, TOUCH_INT=GPIO2; not used in art-frame config)
- **0x6B** — SY6974B battery PMIC
...
- Green user LED: GPIO16; LOW drives the LED on via Q4 transistor (inverted logic) — do not hold GPIO16 LOW during sleep or the LED stays on
- Buzzer: MLT-8530 on GPIO45; driven at 2700 Hz (resonant frequency) with 20 ms pulses for a crisp tactile click
